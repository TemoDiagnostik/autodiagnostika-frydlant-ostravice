(() => {
  ...
  const prepareServiceSection = () => {
    ...
    if (titleRow) {
      ...
      titleRow.innerHTML = `
        <span aria-hidden="true" style="font-size:44px;line-height:1;width:52px;text-align:center;flex-shrink:0;">🔧</span>
        <span style="font-weight:bold;color:#EBDBB1;font-size:20px;line-height:1.25;">
          ${t.serviceTitle} <span style="font-size:16px;">▼</span>
        </span>`;
    }
  ...
  const applyPageContent = () => {
    ...
    if (hero) {
      ...
      const intro = hero.parentElement?.querySelector("p");
      if (intro) intro.innerHTML = `${t.intro}<br>${t.benefits}<br>${t.dropoff}`;
    }

    const mainHeading = Array.from(document.querySelectorAll("h3")).find((heading) =>
    ...
    const priceList = priceContainer?.nextElementSibling?.querySelector("ul");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const isDateDisabled = (date) => {
      ...
      return weekend || bookedSlots.fullDays.includes(toDateKey(date));
    };
    ...
    const resetTimeSelect = () => {
      ...
      option.value = "";
      option.textContent = config.timePlaceholder || "Select time";
    ...
    const calendar = window.flatpickr(dateInput, calendarOptions);
    ...
    if (apiUrl) {
      fetch(apiUrl, { method: "GET", cache: "no-store" })
        ...
        .then((data) => {
          const fullDays = Array.isArray(data?.fullDays) ? data.fullDays.map(String) : [];
          const timeSlots = {};
          const sourceSlots = data?.timeSlots && typeof data.timeSlots === "object" ? data.timeSlots : {};
          ...
          bookedSlots = { fullDays, timeSlots };
    ...
    form.addEventListener("submit", (event) => {
      ...
      try {
        ...
        if (submitButton) {
          ...
          submitButton.textContent = config.submitButtonLabel || "Send";
        }
      }
    });
  });
